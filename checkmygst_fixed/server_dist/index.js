  ...
  app2.post("/api/hsn-lookup", async (req, res) => {
    try {
      ...
      for (const [hsn, data] of Object.entries(HSN_DB)) {
        if (data.description.toLowerCase().split(" ").some((w) => w.length > 3 && q.includes(w))) {
...
Rule: totalITC = igst + cgst + sgst. Use 0 for missing numbers, empty string for missing text.`;
  ...
  app2.post("/api/send-reminder", async (req, res) => {
    try {
...
<body style="font-family:Arial,sans-serif;background:#f8fafc;padding:0;margin:0">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;margin-top:24px">
    <div style="background:#1d4ed8;padding:28px 32px">
      ...
      <p style="color:rgba(255,255,255,0.8);margin:6px 0 0">${businessName || "Your Business"} ${gstin ? "\xB7 " + gstin : ""}</p>
    </div>
    ...
    <div style="padding:16px 32px;border-top:1px solid #f3f4f6;color:#9ca3af;font-size:12px">Sent by CheckMyGST \xB7 You can disable reminders in the app settings.</div>
  </div>
</body>
</html>`;
