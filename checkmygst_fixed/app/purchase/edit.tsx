...
export default function EditPurchaseScreen() {
  ...
  const { purchases, updatePurchase } = useGST();
  ...
  useEffect(() => {
    ...
    if (!purchase) { setNotFound(true); return; }
    setForm({
      supplierName: (purchase as any).supplierName || "",
      supplierGSTIN: (purchase as any).supplierGSTIN || "",
      invoiceNumber: (purchase as any).invoiceNumber || "",
      invoiceDate: purchase.invoiceDate || "",
    });
    // Reconstruct items from purchase
    const purchaseAny = purchase as any;
    if (purchaseAny.items && Array.isArray(purchaseAny.items)) {
      setItems(purchaseAny.items.map((item: any) => ({
        id: generateId(),
      ...
      setItems([{
        id: generateId(),
        description: purchaseAny.description || "",
        hsn: purchaseAny.hsn || "",
        quantity: "1",
        rate: String(purchaseAny.taxableAmount || ""),
        gstRate: purchaseAny.gstRate || 18,
      }]);
    }
  }, [id, purchases]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const firstCalc = calcItem(firstItem);
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.field}>
          ...
          <TextInput style={styles.input} value={form.supplierName} onChangeText={v => updateForm("supplierName", v)} placeholder="e.g. Raj Traders" placeholderTextColor="#9ca3af" />
        </View>
        <View style={styles.field}>
          <Text style={styles.label}>Supplier GSTIN</Text>
          <TextInput style={styles.input} value={form.supplierGSTIN} onChangeText={v => updateForm("supplierGSTIN", v.toUpperCase())} placeholder="15-digit GSTIN" placeholderTextColor="#9ca3af" autoCapitalize="characters" maxLength={15} />
        </View>
              ...
              <View style={styles.field}>
                ...
                <View style={styles.hsnRow}>
                  <TextInput
                    ...
                    onEndEditing={() => handleHSNLookup(item.id, item.description)}
                  />
...
const styles = StyleSheet.create({
  ...
  itemSummary: { flexDirection: "row", justifyContent: "space-between", backgroundColor: "#eff6ff", borderRadius: 8, padding: 10 },
