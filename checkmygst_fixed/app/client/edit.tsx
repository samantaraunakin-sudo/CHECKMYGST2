...
export default function EditClientScreen() {
  ...
  const { clients, updateClient } = useGST();
    ...
    <View style={[styles.container, { paddingTop: topPad }]}>
      ...
      >
        ...
        <View style={styles.field}>
          ...
          <TextInput style={styles.input} placeholder="e.g. Sharma Building Materials" placeholderTextColor={Colors.textMuted} value={form.businessName} onChangeText={(v) => update("businessName", v)} />
        </View>

        <View style={styles.field}>
          <Text style={styles.label}>Owner Name</Text>
          <TextInput style={styles.input} placeholder="e.g. Ramesh Sharma" placeholderTextColor={Colors.textMuted} value={form.ownerName} onChangeText={(v) => update("ownerName", v)} />
        </View>
        ...
        <View style={styles.row}>
          <View style={[styles.field, { flex: 1 }]}>
            ...
            <TextInput style={styles.input} placeholder="10-digit" placeholderTextColor={Colors.textMuted} value={form.phone} onChangeText={(v) => update("phone", v)} keyboardType="phone-pad" maxLength={10} />
          </View>
          <View style={[styles.field, { flex: 1 }]}>
            <Text style={styles.label}>Email</Text>
            <TextInput style={styles.input} placeholder="email@domain.com" placeholderTextColor={Colors.textMuted} value={form.email} onChangeText={(v) => update("email", v)} keyboardType="email-address" autoCapitalize="none" />
          </View>
        </View>
        ...
        <View style={styles.typeGrid}>
          {BUSINESS_TYPES.map((type) => (
            ...
            >
              <Text style={[styles.typeChipText, form.businessType === type && styles.typeChipTextActive]}>{type}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </ScrollView>
