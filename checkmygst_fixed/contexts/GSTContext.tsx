    ...
    try {
      ...
      if (purchasesRes.data) {
        setPurchases(purchasesRes.data.map((p: any) => ({
          ...
          rate: (p.rate && p.rate > 0) ? p.rate : (p.taxable_amount && p.quantity ? p.taxable_amount / p.quantity : 0),
      ...
      if (salesRes.data) {
        setSales(salesRes.data.map((s: any) => ({
          ...
          rate: (s.rate && s.rate > 0) ? s.rate : (s.taxable_amount && s.quantity ? s.taxable_amount / s.quantity : 0),
